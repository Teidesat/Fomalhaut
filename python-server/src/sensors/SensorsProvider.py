...
class SensorsProvider:
    ...
    def try_import_sensor_module(module_name, sensor_name, logger):
        ...
        try:
            ...
            logger.log('Couldnt load module for \'%s\', %s' % (sensor_name, str(e)), Logger.LogLevel.DEBUG)
    ...
    def get_available_sensors(simulate=False, logger=None):
        sensors = []
        if not simulate:
            DS18B20Sensor_ok, DS18B20Sensor   = SensorsProvider.try_import_sensor_module('src.sensors.DS18B20Sensor', 'DS18B20Sensor', logger)
            W1ThermSensor_ok, W1ThermSensor   = SensorsProvider.try_import_sensor_module('w1thermsensor', 'W1ThermSensor', logger)
            CPUTempSensor_ok, CPUTempSensor   = SensorsProvider.try_import_sensor_module('src.sensors.CPUTempSensor', 'CPUTempSensor', logger)
            MMA8451QSensor_ok, MMA8451QSensor = SensorsProvider.try_import_sensor_module('src.sensors.MMA8451QSensor', 'MMA8451QSensor', logger)
            HMC5883LSensor_ok, HMC5883LSensor = SensorsProvider.try_import_sensor_module('src.sensors.HMC5883LSensor', 'HMC5883LSensor', logger)
            BMP180Sensor_ok, BMP180Sensor     = SensorsProvider.try_import_sensor_module('src.sensors.BMP180Sensor', 'BMP180Sensor', logger)
            DHT22Sensor_ok, DHT22Sensor       = SensorsProvider.try_import_sensor_module('src.sensors.DHT22Sensor', 'DHT22Sensor', logger)
