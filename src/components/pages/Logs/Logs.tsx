...
const Logs: FC = () => {
  ...
  const fetchLogs = () => {
    LogsService.getAllLogs()
      ...
      .catch((err) => {
        ...
        setError(
          "Error fetching logs. Please try again later. Explanation: " + err,
        );
        setLogs([]);
      });
  };
  ...
  const filterLogs = () => {
    ...
    LogsService.getLogsFilteredBy(params.toString())
      ...
      .catch((err) => {
        setError(
          "Error fetching filtered logs. Please try again later. Explanation: " +
            err,
        );
        setLogs([]);
      });
      ...
      {selectedLog && (
        <div className="log-modal-overlay" onClick={() => setSelectedLog(null)}>
          <div className="log-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              <strong>Client IP:</strong> {selectedLog.request_client_ip}
            </p>
            <p>
              <strong>Status code:</strong> {selectedLog.request_status_code}
            </p>
            <p>
              <strong>Request path:</strong> {selectedLog.request_path}
            </p>
            <p>
              <strong>User:</strong> {selectedLog.request_user}
            </p>
            <p>
              <strong>Exception:</strong> {selectedLog.exception_type}
            </p>
            <p>
              <strong>Stack trace:</strong>
            </p>
            <pre style={{ whiteSpace: "pre-wrap" }}>
              {selectedLog.exception_stack_trace}
            </pre>
            <p>
              <strong>Extra:</strong>
            </p>
